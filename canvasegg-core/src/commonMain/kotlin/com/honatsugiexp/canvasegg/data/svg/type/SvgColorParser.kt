...
import com.honatsugiexp.canvasegg.common.util.doubleColor
...
object SvgColorParser {
    ...
    /**
     * ComposeColorをlch(L, C, H[, A])形式の文字列に変換します。
     * (Lab to LCH 変換)
     */
    fun toLch(color: ComposeColor): String {
        ...
        // a, b から C, H へ
        val chromaC = kotlin.math.sqrt(greenToRedA.pow(2) + blueToYellowB.pow(2))
        var hueH = toDegrees(kotlin.math.atan2(blueToYellowB, greenToRedA))
        if (hueH < 0) hueH += 360f
    ...
    fun toOklch(color: ComposeColor): String {
        ...
        // a, b から C, H へ
        val chromaC = kotlin.math.sqrt(aAxis.pow(2) + bAxis.pow(2))
        var hueH = toDegrees(kotlin.math.atan2(bAxis, aAxis))
        if (hueH < 0) hueH += 360f
