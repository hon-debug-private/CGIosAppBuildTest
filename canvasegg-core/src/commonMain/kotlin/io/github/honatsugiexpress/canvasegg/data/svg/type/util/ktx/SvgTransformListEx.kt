...
internal fun DrawTransform.applySvgTransform(transformList: SvgTransformList) {
    transformList.reversed().forEach { transform ->
        when (transform) {
            ...
            is SvgTransform.SkewX -> transform(
                createSkewMatrix(
                    transform.angle,
                    true
                )
            )
            is SvgTransform.SkewY -> transform(
                createSkewMatrix(
                    transform.angle,
                    false
                )
...
internal fun Path.applySvgTransform(transformList: SvgTransformList) {
    transformList.forEach { transform ->
        val matrix = when (transform) {
            ...
            is SvgTransform.SkewX -> createSkewMatrix(
                transform.angle,
                true
            )
            is SvgTransform.SkewY -> createSkewMatrix(
                transform.angle,
                false
            )
